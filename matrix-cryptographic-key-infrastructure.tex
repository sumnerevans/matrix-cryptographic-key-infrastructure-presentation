...
\begin{frame}{Big Picture: Identity: Device Verification}
    ...
    \note[item]{
        ...
        another device for our own user.
    }
\end{frame}
...
\note{
    We're going to start by discussing how we get keys from one device to
    another. This process is generally called ``key sharing''.
}
...
\begin{frame}{Encrypted Olm Events}
    ...
    \note[item]{
        ...
        to other users' devices via these \texttt{m.room\_key} events and our
        own devices using via to-device events via Olm. We can also request keys
        by sending \texttt{m.room\_key\_request} events to our own
        \textbf{verified} devices and the other devices can respond using
